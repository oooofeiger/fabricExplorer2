...
class OrdererDeploy extends React.Component {
  ...
  handleSubmit = (e) => {
    ...
    createId && form.validateFields(['genesisBlock'],(err, values) => {
      if (!err) {
        let formData = new FormData();
        formData.append('genesisBlock', values['genesisBlock'].file);
        dispatch({
          type: 'network/handleCreateOrderer',
  ...
  render() {
    ...
    const { caServerName, blockFileList, createId, ordererCertArr, userCertArr } = this.state;

    let CAUserArr = []; //CA用户数组
    let peerIdArr = []; //peer类型用户ID数组
    let userIdArr = []; //user类型用户ID数组
    caServerName && CAUser && CAUser.map(ele=>{
      if(CAUserArr.indexOf(ele.id)<0){
        CAUserArr.push(ele.id)
      }
      if(ele.type === 'orderer' && ele.serverName === caServerName && peerIdArr.indexOf(ele.id) < 0){
        peerIdArr.push(ele.id)
      }
      if(ele.type === 'user' && ele.serverName === caServerName && userIdArr.indexOf(ele.id) < 0){
        userIdArr.push(ele.id)
      }
    })

    let ordererAki = []; //Orderer签名证书身份标识
    let userAki = [];  //Orderer用户证书身份标识
    ordererCertArr && ordererCertArr.map((item)=>{
      if(ordererAki.indexOf(item.authorityKeyIdentifier)<0){
        ordererAki.push(item.authorityKeyIdentifier)
      }
    })

    userCertArr && userCertArr.map((item)=>{
      if(userAki.indexOf(item.authorityKeyIdentifier)<0){
        userAki.push(item.authorityKeyIdentifier)
      }
    })
    ...
    return (
      ...
      <FormItem {...formItemLayout} style={{marginBottom:10}} className={styles.createCALabel} label="节点配置"></FormItem>
        ...
        <FormItem wrapperCol={{ span: 8, offset: 8 }}>
          <Button disabled={!createId} type="primary" block htmlType="submit">
            确定
          </Button>
        </FormItem>
      </Form>
