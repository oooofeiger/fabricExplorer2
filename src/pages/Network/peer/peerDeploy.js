...
class DeployPeer extends React.Component {
  ...
  handleSubmit = (e) => {
    ...
    form.validateFields((err, values) => {
      if (!err) {
        ...
        values.extraHosts = values.extraHosts.split(',')
        dispatch({
  ...
  render() {
    ...
    const { peerName, couchdb, caServerName, peerCertArr, userCertArr } = this.state;
    ...
    caServerName && CAUser && CAUser.map(ele=>{
      ...
      if(ele.type === 'peer' && ele.serverName === caServerName && peerIdArr.indexOf(ele.id) < 0){
    ...
    let peerAki = []; //peer签名证书身份标识
    let userAki = [];  //peer用户证书身份标识
    peerCertArr && peerCertArr.map((item)=>{
      if(peerAki.indexOf(item.authorityKeyIdentifier)<0){
        peerAki.push(item.authorityKeyIdentifier)
      }
    })
