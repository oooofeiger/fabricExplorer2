  ...
  class UserCert extends React.Component {
    ...
    download = (serial, aki) => {
        const params = {
            ...
            fileName: `${serial}.zip`,
            url: `http://${host}/ca/cert/download?serial=${serial}&aki=${aki}`,
            type: 'get'
        }
        downloadFile(params)
    }
    ...
    render(){
        ...
        getCAUser && getCAUser.map((ele,i)=>{
            ...
            if(userIdArr.indexOf(ele.id)<0 && ele.serverName === serverName){
        ...
        return (
            <PageHeaderLayout detailInfo={detailInfo} toggleSwitch={<SelectServer changeSelectServer={this.changeSelectServer} serverData={serverArr} serverName={serverName} />} logo={org}>
                ...
                <TabPane
                    ...
                    key="1"
                >
                    <TabPaneCon children={<Table bordered dataSource={getCertInfo} columns={columns} />} select={<SelectUser userId={userId} changeSelectUser={this.changeSelectUser} userIdArr={userIdArr}/>} title="证书列表"/>
