...
@connect(({ CAManager, global, host, loading }) => {
  return {
    CAManager,
    global,
    host,
...
class CAManager extends React.Component {
  ...
  componentDidUpdate(){
    ...
    const { deleteCA } = CAManager;
    const { update, deleteStamp, serverName, deleteFlag } = this.state;
  ...
  render() {
    ...
    const { activeKey, serverName, modalTitle, isRootCA, current } = this.state;

    const detailInfo = (
      <div className={styles.peer}>CA管理</div>
    );

    function SelectCA({serverName, getCA, changeSelectCA}){
        return <Select
                    showSearch
                    style={{ width: 200 }}
                    placeholder="请选择服务"
