...
class HostUpdate extends React.Component {
  ...
  render() {
    ...
    const { hostData, oneHost } = host;
    const { getFieldDecorator } = form;
    const { couldSubmit } = this.state;
      ...
      <Form encType="multipart/form-data" onSubmit={this.handleSubmit}>
        <FormItem {...formItemLayout} label="编辑">
          ...
          <FormItem {...formItemLayout} hasFeedback={true} label="docker端口">
            ...
            })(<Input disabled={!couldSubmit} placeholder="请输入服务端口" />)}
          </FormItem>

          <FormItem {...formItemLayout} label="是否使用tls">
            {getFieldDecorator('tlsEnable', {
              initialValue: oneHost ? oneHost.tlsEnable : true,
            })(
              <RadioGroup disabled={!couldSubmit} onChange={this.changeTls}>
          ...
          <FormItem {...formItemLayout} label="protocol">
            {getFieldDecorator('protocol', {
              initialValue: oneHost ? oneHost.protocol : 'tcp',
            })(
              <RadioGroup disabled={!couldSubmit} onChange={this.changeTls}>
          ...
          <FormItem wrapperCol={{ span: 8, offset: 8 }}>
            <Button type="primary" disabled={!couldSubmit} block htmlType="submit">
              更新
            </Button>
          </FormItem>
        </Form>
